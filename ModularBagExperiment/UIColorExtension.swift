...
extension UIColor {
    ...
    public class func hexColor(_ hex: String) -> UIColor {
        ...
        guard trimmedColor.hasPrefix("#") == true else {
            ...
            print("Hex Color String do not have initial hash: \(trimmedColor)")
            return color
        }
    ...
    fileprivate class func colorWithShorthandHex(_ hexColor: String) -> UIColor {
        ...
        let red = hexColor.first!
        let green = hexColor[hexColor.index(after: hexColor.startIndex)]
        let blue = hexColor[hexColor.index(before: hexColor.endIndex)]
        
        let hex = String([red, red, green, green, blue, blue])
